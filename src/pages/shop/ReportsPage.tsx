...
import DateRangePicker from "../../components/shop/reports/DateRangePicker.tsx";
import {QuickRangeKey} from "../../components/shop/reports/QuickDateRangeButtons.tsx";
import {startOfWeek, startOfMonth, subMonths, startOfYear, subDays} from 'date-fns';
import {toDate, format} from 'date-fns-tz';
import ReportSidebar, {ReportType} from "../../components/shop/reports/ReportSidebar.tsx";
import SalesReport from "../../components/shop/reports/SalesReport.tsx";
import {getCashSessionsForPeriod, getPurchasesForPeriod} from "../../services/shop/dashboardService.ts";
import {getExpensesForPeriod} from "../../services/shop/expenseServices.ts";
import PurchasesReport from "../../components/shop/reports/PurchasesReport.tsx";
import ProfitReport from "../../components/shop/reports/ProfitReport.tsx";
import SessionsReport from "../../components/shop/reports/SessionsReport.tsx";
...
const ReportsPage: FC = () => {
    ...
    const initialDateString = activeShop?.timezone
        ? format(new Date(), 'yyyy-MM-dd', {timeZone: activeShop.timezone})
        : new Date().toISOString().split('T')[0];
    const [startDate, setStartDate] = useState(initialDateString);
    const [endDate, setEndDate] = useState(initialDateString);

    const handleFetchReport = async () => {
        if (!activeShop?.id || !activeShop.timezone) {
            console.warn("No hay heladería activa o no tiene zona horaria definida.");
            return;
        }
        setLoading(true);
        try {
            const start = toDate(`${startDate}T00:00:00`, {timeZone: activeShop.timezone});
            const end = toDate(`${endDate}T23:59:59`, {timeZone: activeShop.timezone});

            if (activeReport === 'sales') {
                const [salesData, ingredientsData] = await Promise.all([
                    getSalesByDateRange(activeShop.id, start, end),
                    getIngredients(activeShop.id) // Necesario para el modal de detalles
                ]);
                setSales(salesData);
                setIngredients(ingredientsData);
            } else if (activeReport === 'sessions') {
                const sessionsData = await getCashSessionsForPeriod(activeShop.id, start, end);
                setSessions(sessionsData);
            } else if (activeReport === 'purchases') {
                const purchasesData = await getPurchasesForPeriod(activeShop.id, start, end);
                ...
                const [salesData, purchasesData, sessionsData, expensesData] = await Promise.all([
                    getSalesByDateRange(activeShop.id, start, end),
                    getPurchasesForPeriod(activeShop.id, start, end),
                    getCashSessionsForPeriod(activeShop.id, start, end),
                    getExpensesForPeriod(activeShop.id, start, end)
                ]);
                setSales(salesData);
                setPurchases(purchasesData);
                setSessions(sessionsData);
    ...
    const handleQuickRangeSelect = (range: QuickRangeKey) => {
        if (!activeShop?.timezone) return;

        // Obtenemos la fecha "ahora" pero en la zona horaria de la tienda
        const nowInShopTimezone = toDate(new Date(), {timeZone: activeShop.timezone});
        ...
        setStartDate(format(newStartDate, 'yyyy-MM-dd', {timeZone: activeShop.timezone}));
        setEndDate(format(newEndDate, 'yyyy-MM-dd', {timeZone: activeShop.timezone}));
    };
