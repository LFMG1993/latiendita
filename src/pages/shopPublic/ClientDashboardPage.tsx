...
import {getClientFinancials, ClientFinancials} from '../../services/shopPublic/clientService';
import {getClientOrders} from '../../services/shop/orderService';
import {Order} from '../../types/order.types';
import {PaymentMethod, DebtPaymentRequest} from '../../types';
import {getActivePaymentMethods} from '../../services/shop/paymentMethodServices';
import {getClientDebtPayments, createDebtPaymentRequest} from '../../services/shop/debtPaymentService';
...
const ClientDashboardPage: FC = () => {
    ...
    const handleChangePassword = async () => {
        ...
        setProfileSuccess('');
        try {
            // TODO: Connect to Go backend when PUT /api/users/password is implemented
            console.warn("Change password not fully implemented in frontend without firebase");
            setCurrentPassword('');
            setNewPassword('');
            setConfirmNewPassword('');
            setProfileSuccess('Contraseña cambiada correctamente (simulado).');
        } catch (err: any) {
    ...
    const handleCheckout = async () => {
        ...
        const urlShopId = searchParams.get('shopId');
        const localShopId = localStorage.getItem('last_shop_id');
        const shopId = urlShopId || localShopId || user.shopIds?.[0];
