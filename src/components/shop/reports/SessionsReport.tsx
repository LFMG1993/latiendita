...
import {getSalesBySessionId} from "../../../services/shop/saleServices.ts";
import {getPurchasesForSession} from "../../../services/shop/dashboardService.ts";
import {getExpensesForSession} from "../../../services/shop/expenseServices.ts";
...
const SessionsReport: FC<SessionsReportProps> = ({sessions, loading}) => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            if (!selectedSession || !activeShop?.id) return;

            setDetailsLoading(true);
            try {
                const [sales, purchases, expenses] = await Promise.all([
                    getSalesBySessionId(activeShop.id, selectedSession.id),
                    getPurchasesForSession(activeShop.id, selectedSession.employeeId, new Date(selectedSession.startTime), new Date(selectedSession.endTime!)),
                    getExpensesForSession(activeShop.id, selectedSession.id)
                ]);
